      ...
      <div className="h-full flex flex-col bg-slate-50 font-sans">
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200/80 bg-white shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-800 leading-tight">IEP Generator</h2>
              <p className="text-xs text-slate-400">Select a student with active IEP status</p>
            </div>
          </div>
          <div className="relative w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl text-sm focus:ring-4 focus:ring-cyan-500/20 outline-none bg-white"
              ...
              {filteredStudents.map(s => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 hover:bg-cyan-50/50 transition-colors flex items-center gap-3 group"
                  >
                    <EditableStudentName 
                      studentId={s.id} 
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-sm text-slate-800">{formatStudentLabel(s)}</div>
                      <div className="text-xs text-slate-400">Grade {s.gradeLevel} &middot; {s.unitName}</div>
                    </div>
                    {urgency && (
                      <div className={`text-xs font-bold px-2 py-1 rounded-full shrink-0 ${urgency === 'overdue' ? 'bg-red-50 text-red-600' : urgency === 'soon' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
